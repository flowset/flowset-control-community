/*
 * Copyright (c) Haulmont 2026. All Rights Reserved.
 * Use is subject to license terms.
 */

package io.flowset.control.test_support.ui.view.processinstance.detail.tab;

import io.flowset.control.test_support.ui.component.GridContextMenu;
import io.jmix.masquerade.TestComponent;
import io.jmix.masquerade.component.DataGrid;
import io.jmix.masquerade.sys.Composite;
import lombok.Getter;
import org.openqa.selenium.By;

import static io.flowset.control.test_support.ui.UiTestSupport.getRowByCellContent;
import static io.flowset.control.test_support.ui.UiTestSupport.openGridContextMenu;
import static io.jmix.masquerade.JConditions.VISIBLE;
import static io.jmix.masquerade.JSelectors.byPath;

/**
 * Wrapper for the Decisions tab fragment inside the History tab of the Process instance detail view.
 * Source component: {@link io.flowset.control.view.processinstance.history.HistoryDecisionsFragment}
 */
@Getter
public class HistoryDecisionsTabFragment extends Composite<HistoryDecisionsTabFragment> {

    public static final By ID_BUTTON_BY = byPath("root", "idBtn");

    public static final int DECISION_INSTANCE_ID_COLUMN_INDEX = 0;
    public static final int ACTIVITY_ID_COLUMN_INDEX = 1;
    public static final int DECISION_DEFINITION_ID_COLUMN_INDEX = 2;
    public static final int EVALUATION_TIME_COLUMN_INDEX = 3;

    @TestComponent(path = "historyTasksGrid")
    private DataGrid historyDecisionsGrid;

    /**
     * Finds the row in the grid for a decision instance related to the specified activity identifier.
     *
     * @param activityId an activity identifier from BPMN XML
     * @return a grid row or exception thrown if not found
     */
    public DataGrid.Row getRowByActivityId(String activityId) {
        return getRowByCellContent(historyDecisionsGrid, ACTIVITY_ID_COLUMN_INDEX, activityId);
    }

    /**
     * Finds the row in the grid for a decision instance related to the specified identifier.
     *
     * @param decisionInstanceId a decision instance identifier
     * @return a grid row or exception thrown if not found
     */
    public DataGrid.Row getRowByDecisionInstanceId(String decisionInstanceId) {
        return getRowByCellContent(historyDecisionsGrid, DECISION_INSTANCE_ID_COLUMN_INDEX, cell -> {
            String idBtnText = cell.getCellContent()
                    .find(ID_BUTTON_BY)
                    .getText();
            return idBtnText.equals(decisionInstanceId);
        });
    }

    /**
     * Selects the row in the grid by the specified activity identifier.
     *
     * @param activityId an activity identifier
     */
    public void selectRowByActivityId(String activityId) {
        getRowByActivityId(activityId)
                .getCellByIndex(ACTIVITY_ID_COLUMN_INDEX)
                .getCellContent()
                .shouldBe(VISIBLE)
                .click();
    }

    /**
     * Opens the context menu for the decision instance data grid.
     *
     * @return opened grid context menu
     */
    public GridContextMenu openDecisionsGridContextMenu() {
        return openGridContextMenu(historyDecisionsGrid);
    }
}
